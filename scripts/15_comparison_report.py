#!/usr/bin/env python3
"""Build the side-by-side manuscript-vs-pipeline comparison report.
...
# Dr Naeem's is an exact half/quarter-scale render of the same panel) and by
...
<figure class="pair">
  ...
  <div class="imgs">
    <div class="side"><div class="sh">Dr Naeem's manuscript <em>{a.width}&times;{a.height}px · {a_dpi} dpi</em></div>
...
<section class="vals">
  ...
  <div class="tw"><table>
    ...
    css = CSS
    page = f"""<title>Manuscript vs Pipeline — Cardiac CT Readability</title>
<style>{css}</style>
<header class="masthead">
  <p class="eyebrow">Reproducibility cross-check</p>
  <h1>Dr Naeem's manuscript, checked line by line against this repository</h1>
  <p class="standfirst">Every value reported in
  <em>Naeem_final_clean_cardiac_CT_readability.docx</em> was parsed out of the document,
  recomputed from the study data, and compared. Nothing is transcribed by hand and nothing
  is read back from our own reports — the manuscript is the independent reference.</p>
...
<section class="verdict">
  ...
  <p>Of <strong>{total}</strong> values Dr Naeem reports, <strong>{n_pass}</strong> reproduce
  ...
  <h3>The one genuine difference</h3>
  <ul class="findings">{fail_html}</ul>
  <p class="note">Everything structurally adjacent to this value reconciles exactly — 104 rater-pairs,
  82/75/88&#37; exact agreement, and all three quadratic-weighted &kappa; values computed from the
  <em>same</em> rater&times;item matrix. That rules out the cohort, the pairing and the pivot, and
  localises the difference to the AC1 estimator on one axis. Four AC1 variants were tested and none
  reproduces 0.77; the estimator was deliberately <em>not</em> tuned to hit the target. The direction
  favours the paper: 0.805 is <em>higher</em> agreement than 0.77.</p>
