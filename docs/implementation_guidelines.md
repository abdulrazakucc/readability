...
2. **Lock model versions in config.** Use specific snapshot IDs where the API supports them (e.g., `claude-opus-4-7`, `gpt-4o-2024-08-06`, `gemini-1.5-pro-002`).
3. **Temperature = 0** (or the lowest stable value) for the rewrite arm. We want reproducibility, not creativity.
...
Record `textstat.__version__` with every score (the algorithms have shifted in past releases — pin and record).
...
- `data/manifest.csv` (text, small, central).
- `reports/` outputs that the manuscript cites.
- `requirements.txt` and `requirements.lock`.

Out (gitignored):
- `.venv/`
- `.env` (real credentials)
- `data/raw/` (potentially large, may have copyright considerations)
- `data/cleaned/`, `data/rewrites/` (derivable from raw + code)
- `__pycache__/`, `.pytest_cache/`, `.ipynb_checkpoints/`
- API response cache directories

When in doubt about copyright: store raw HTML privately, share only the URLs and the manifest publicly.
